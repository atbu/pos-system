...
export const GET = async () => {
  ...
  try {
    ...
    // Creates an object containing the date exactly 30 days ago (i.e. a month).
    const lastMonthDate = new Date(currentDate.getTime() 
            - 30 * 24 * 60 * 60 * 1000); 

    // Creates a Prisma request to get all tips from the last 30 days.
    ...
    return new NextResponse(
      JSON.stringify( { message: "Something went wrong while fetching all tips from the last 30 days." } ),
      { status: 500 }
    )
